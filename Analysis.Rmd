...
ggplot(train, aes(OutcomeType)) +
        ...
        ggtitle("Percentage of animals by outcome") +
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
#outcomes <- factor(train$OutcomeType, levels = unique(train$OutcomeType))
train.copy$AgeuponOutcome[grep("1 week", train.copy$AgeuponOutcome)] <- "1 week"

train.copy %>% group_by(AgeuponOutcome) %>%
    #ggplot(aes(x = reorder(AgeuponOutcome, outcomes == "Adoption", sum), fill = OutcomeType)) +
     ggplot(aes(x = AgeuponOutcome, fill = OutcomeType)) +
        geom_bar(stat = "count") + 
        facet_wrap(~AnimalType, nrow = 2) +
        ggtitle("Age of animals by outcome ordered by number of animals adopted") +
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
train.copy$NameLength[train$Name != 0] <- str_length(train$Name)
...
train.cats.noname <- filter(train, AnimalType == "Cat" & Name == 0 & OutcomeType == "Transfer")
train.transfers <- filter(train, OutcomeType == "Transfer")
```

From the bar chart, we can see that the name length follow the normal distribution if we exclude length of 0. Animals having no name are mostly transfered but for cats, this is very significant. Transferred cats having no name represent `r nrow(train.cats.noname) / train.n * 100` % of the train set and `r nrow(train.transfers) / train.n * 100` % of all the transferred animals of the train set. This is clearly an insight to consider. Therefore, we transform string values of the `Name` feature in boolean values telling if the animal has a name = 1 or has no name = 0.
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
train.copy %>% group_by(SexuponOutcome) %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
train.copy$Days <- day(train.date)
...
For the dogs, Shih Tzu are mostly transferred and less adopted. Pit bulls, Labrador Retreiver and Chihuahua Shorthair are the most common dogs for outcomes. For cats, the domestic shorthair is the most frequent and represent `r nrow(filter(train, grepl("Domestic Shorthair", Breed) & AnimalType == "Cat")) / nrow(filter(train, AnimalType == "Cat")) * 100` % of the cats in the train set.
...
Intact animals have a low percentage of adoptions. They represent `r intacts.adoption` % of intact animals (which are `r intacts`).
...
Sterile animals show a better adoption ratio. They represent `r steriles.adoption` % of sterile animals (which are `r steriles`). This is an increase of `r steriles.adoption - intacts.adoption` % compared to the intact ones.
