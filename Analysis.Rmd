...
train.cats.noname <- filter(train.copy, AnimalType == "Cat" & Name == 0 & OutcomeType == "Transfer")
train.transfers <- filter(train.copy, OutcomeType == "Transfer")
```
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
## Get number of adoptions per hours for each weekday.
train.adopted.list <- train.copy %>% 
    group_by(Hour, OutcomeType, WeekDay) %>% 
    summarise(Total = n()) %>% 
    filter(OutcomeType == "Adoption")
train.adopted.list <- train.adopted.list[with(train.adopted.list, order(-Total)), ]
...
train.adopted.list %>%
    ggplot(aes(x = Hour, y = Total)) +
        geom_bar(stat = "identity") + 
        facet_wrap(~WeekDay, ncol = 3, nrow = 3) +
        ggtitle("Number of adopted animals for each hour by Weekday") +
        labs(y = "Number of animals", x = "Hours")
```
